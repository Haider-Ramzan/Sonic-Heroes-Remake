...
Level2::Level2(int screenWidth)
    : Level("2", "Data/bg_ice_cap.png", screenWidth) {
    //set level 1 accelerations and stuff
    player->setAcceleration(1.0f);
    player->setDeacceleration(-0.2f);
    ...
    enemyArray[0] = L1.createEnemy("CrabMeat", 1342 + 2000, 767);
    enemyArray[1] = (Motobug*)L1.createEnemy("MotoBug", 6339.5 + 500, 767);
    enemyArray[2] = (BatBrain*)L1.createEnemy("BatBrain", 6339.5, 500);
    enemyArray[3] = (BeeBot*)L1.createEnemy("BeeBot", 1342, 300);
    enemyArray[4] = L1.createEnemy("CrabMeat", 6339.5 + 3500, 767);
...
void Level2::run(sf::RenderWindow& window, int& ringScore) {
    sf::Clock deltaClock;
    sf::Event ev;
    Clock clk_spikes;
...
bool Level2::isWinner() {
    if (player->getPosX() > 245 * 64) {
        return true;
    }
